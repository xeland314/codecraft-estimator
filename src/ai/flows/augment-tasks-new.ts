...
Return the augmented tasks with adjusted time estimates as a clear, well-formatted string.`;
...
Augmentation Prompt: ${input.prompt}`;
  ...
  try {
    ...
    const augmentedTasks = response.response.text();
    const validated = AugmentTasksOutputSchema.parse({ augmentedTasks });
    return validated;
  } catch (error) {
